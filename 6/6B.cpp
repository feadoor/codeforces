...
int main() {
    int n, m; cin >> n >> m;
    char c; cin >> c;
    char room[n][m]; REP(i, 0, n) REP(j, 0, m) cin >> room[i][j];
    set<char> deps;

    REP(i, 0, n) REP(j, 0, m) if (room[i][j] != '.' && room[i][j] != c) {
        if (i > 0 && room[i - 1][j] == c || i < n - 1 && room[i + 1][j] == c || j > 0 && room[i][j - 1] == c || j < m - 1 && room[i][j + 1] == c) deps.insert(room[i][j]);
    }
