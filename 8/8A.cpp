...
int main() {
    ...
    auto fit = search(ALL(s), ALL(one));
    fit = search(fit + SZ(one), s.end(), ALL(two));
    bool forward = (fit != s.end());

    auto bit = search(RALL(s), ALL(one));
    bit = search(bit + SZ(one), s.rend(), ALL(two));
    bool backward = (bit != s.rend());
