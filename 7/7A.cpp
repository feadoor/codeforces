...
int main() {
    vs board(8); AREP(&s, board) cin >> s;
    int rs = 0, cs = 0;
    REP(i, 0, 8) {
        bool fullr = true; bool fullc = true;
        REP(j, 0, 8) {
            if (board[i][j] == 'W') fullr = false;
            if (board[j][i] == 'W') fullc = false;
        }
        if (fullr) ++rs; if (fullc) ++cs;
    }
    if (rs == 8) cout << rs << endl; else cout << rs + cs << endl;
}