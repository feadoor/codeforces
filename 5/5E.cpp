...
int main() {
    ...
    int n; cin >> n;
    vi hills(n); AREP(&h, hills) cin >> h;
    rotate(hills.begin(), MAXE(hills), hills.end());

    vi peaks; vb check(n, false); ll pairs = 0;
    REP(i, 0, SZ(hills)) { int h = hills[i];
        auto uit = UB(RALL(peaks), h); pairs += (peaks.end() - uit.base() + (uit == peaks.rend() ? 0 : 1));
        if (uit < peaks.rend() - 1) check[i] = true;
        auto lit = LB(RALL(peaks), h); peaks.erase(lit.base(), peaks.end());
        peaks.PB(h);
    }

    int largest = 0;
    RREP(i, 0, SZ(hills)) {
        if (check[i] && hills[i] >= largest) ++pairs;
        largest = max(largest, hills[i]);
    }
